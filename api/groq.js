...
export default async function handler(req, res) {
  ...
  const purpose = ['translation', 'smart_search', 'investment', 'fair_price'].includes(req.body.purpose) ? req.body.purpose : 'chat'
  ...
  if (purpose === 'investment' || purpose === 'fair_price') {
    if (!authUser) {
      ...
      return res.status(401).json({ error: { message: authMsg } })
    }
  ...
  const cacheProperty = purpose === 'investment' || purpose === 'fair_price' ? req.body.property : null
  const cacheFingerprint = cacheProperty
    ? purpose === 'fair_price'
      ? computeFairPriceFingerprint(req.body)
      : computeCacheFingerprint(req.body)
    : ''
