...
function SuccessModal({ agentWa, onClose }) {
  ...
  const waLink = `https://wa.me/${agentWa}?text=${encodeURIComponent(t('sellProperty.success.view_agent'))}`

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-brand-surface rounded-3xl w-full max-w-sm p-8 text-center shadow-xl border border-brand-border">
        <div className="w-16 h-16 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-5">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        </div>
        <h2 className="text-xl font-bold text-brand-text mb-2">{t('sellProperty.success.title')}</h2>
        <p className="text-sm text-brand-muted leading-relaxed">
          {t('sellProperty.success.description')}
        </p>
        <div className="mt-7 space-y-3">
          <a
            ...
            className="w-full py-3.5 rounded-xl font-bold text-sm text-white bg-emerald-600 hover:brightness-90 transition-all flex items-center justify-center gap-2 shadow-sm"
          >
            ...
            {t('sellProperty.success.view_agent')}
          </a>
          <button
            type="button"
            onClick={onClose}
            className="w-full py-3 rounded-xl font-medium text-sm text-brand-muted bg-brand-bg hover:bg-brand-border transition-colors border border-brand-border"
          >
            {t('sellProperty.success.back_home')}
          </button>
        </div>
      </div>
    </div>
