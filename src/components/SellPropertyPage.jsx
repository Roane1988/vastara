...
const STEPS = ['Kontak', 'Spesifikasi', 'Lokasi', 'Dokumen', 'Review']
...
function LocationPicker({ position, onPositionChange }) {
  ...
  return position ? <Marker position={position} draggable={true} eventHandlers={{ dragend: (e) => { const m = e.target; onPositionChange([m.getLatLng().lat, m.getLatLng().lng]) } }} /> : null
}
...
export default function SellPropertyPage() {
  ...
  const canProceed = () => {
    switch (step) {
      case 0: return form.whatsapp.trim().length >= 10
      case 1: return form.sqm && form.bedrooms && form.bathrooms && form.description.trim()
      case 2: return form.address.trim()
      case 3: return !!fileUrl || file === null
      default: return true
    }
  }
  ...
  const handleFileSelect = async (e) => {
    ...
    const filePath = `listings/${Date.now()}_${selectedFile.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="h-56 sm:h-72 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
                <MapContainer
        ...
        return (
          <div className="space-y-5">
            <div>
              ...
              {!file ? (
                <label className="flex flex-col items-center justify-center w-full py-8 px-4 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-800/50 cursor-pointer hover:border-orange-400 dark:hover:border-orange-500 transition-colors">
                  <UploadIcon />
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
              {[
                ...
                { label: 'Luas Bangunan', value: `${form.sqm} m²` },
                { label: 'Kamar Tidur', value: `${form.bedrooms}` },
                { label: 'Kamar Mandi', value: `${form.bathrooms}` },
              ...
              {fileUrl && (
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-500 dark:text-slate-400">Dokumen</span>
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
              <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                Dengan mengirimkan data ini, Anda menyetujui bahwa informasi properti Anda akan diverifikasi oleh tim Vastara sebelum dipublikasikan.
              </p>
            </div>
          </div>
        )
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
          ...
          <div className="mt-8 w-full max-w-xs space-y-3">
            <a
              ...
              className="w-full py-3.5 rounded-xl font-bold text-white bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20"
            >
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col">
      <header className="sticky top-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md z-30 pt-12 pb-3 px-5 border-b border-slate-100 dark:border-slate-800">
      ...
      <div className="flex-1 px-5 pt-6 pb-4 overflow-y-auto">
        <h2 className="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-5">
          {STEPS[step]}
        </h2>
        {renderStepContent()}
      </div>
        ...
        <div className="flex gap-3">
          {step > 0 && (
            <button
              type="button"
              onClick={prevStep}
              className="flex-1 py-3.5 rounded-xl font-medium text-sm text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
            >
              Kembali
            </button>
          )}
          {step < STEPS.length - 1 ? (
            <button
              ...
              onClick={nextStep}
              disabled={!canProceed()}
              className="flex-1 py-3.5 rounded-xl font-bold text-sm text-white bg-orange-500 hover:bg-orange-600 active:bg-orange-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Lanjut
            </button>
          ) : (
            <button
              ...
              className="flex-1 py-3.5 rounded-xl font-bold text-sm text-white bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
