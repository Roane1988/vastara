...
import { MessageSquare, ChevronDown, ChevronRight, X, ChevronLeft, Calendar, Share2, MapPin, Tag, TrendingDown, Flag, CheckCircle2, AlertTriangle, Wallet, Heart, Images } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import { getFavorites, toggleFavorite as toggleFav } from '../utils/favorites'
import { normalizeWhatsAppNumber, isValidWhatsAppNumber } from '../utils/whatsapp'
...
export default function PropertyDetailPage() {
  ...
  const rawWa = property?.seller_whatsapp
  const waNumber = rawWa && isValidWhatsAppNumber(rawWa) ? normalizeWhatsAppNumber(rawWa) : ''
  const waLink = waNumber
    ? `https://wa.me/${waNumber}?text=${encodeURIComponent(
        `Halo, saya tertarik dengan properti "${property.title || ''}" ini. Apakah masih tersedia?`
      )}`
    : null
    ...
    <div className="min-h-screen bg-brand-bg flex flex-col">
            ...
            <div className="rounded-2xl border border-brand-border bg-white p-5">
              ...
              {property.seller_id && (
                ...
                >
                  ...
                  {lang === 'en' ? 'Get full address in chat' : 'Dapatkan alamat lengkap via chat'}
                </button>
              )}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-5 py-2.5 flex items-center justify-between">
          ...
          {property.seller_id && (
            <button
              type="button"
              onClick={handleChatClick}
              className="shrink-0 flex items-center gap-2 py-2 px-4 rounded-lg text-sm font-bold text-white bg-brand-primary hover:bg-brand-primary/90 transition-colors active:scale-[0.97]"
            >
              <MessageSquare size={16} />
              Chat
            </button>
      ...
      {property.seller_id && (
        <div className={`fixed bottom-0 left-0 right-0 w-full z-50 lg:hidden transition-transform duration-300 ease-in-out ${showFloatingBtn ? 'translate-y-0' : 'translate-y-[150%]'}`}>
          <div className="bg-brand-surface/95 backdrop-blur-md border-t border-brand-border px-4 py-3 flex items-center gap-2.5">
            <div className="min-w-0 shrink">
              <p className="text-[10px] font-medium uppercase tracking-wide text-brand-muted">{lang === 'en' ? 'Price' : 'Harga'}</p>
              <p className="text-base font-extrabold text-brand-text leading-tight truncate">{displayPrice}</p>
              {Number(property?.price) > 0 && Number(property?.area_sqm) > 0 && !isRent && (
                <p className="text-[10px] text-brand-muted truncate">{formatPrice(Math.round(Number(property.price) / Number(property.area_sqm)))}/m²</p>
              )}
            </div>
            <button
              type="button"
              onClick={toggleSave}
              aria-pressed={isSaved}
              aria-label={isSaved ? (lang === 'en' ? 'Remove from favorites' : 'Hapus dari favorit') : (lang === 'en' ? 'Save to favorites' : 'Simpan ke favorit')}
              className={`shrink-0 w-11 h-11 flex items-center justify-center rounded-xl border transition-all active:scale-[0.95] ${
                isSaved
                  ? 'bg-brand-highlight text-brand-accent border-brand-accent/30'
                  : 'bg-white text-brand-muted border-brand-border hover:border-brand-accent hover:text-brand-accent'
              }`}
            >
              <Heart size={18} className={isSaved ? 'fill-brand-accent' : ''} />
            </button>
            {waLink && (
              <a
                href={waLink}
                ...
                aria-label="WhatsApp"
                title="WhatsApp"
                className="shrink-0 w-11 h-11 flex items-center justify-center rounded-xl border border-brand-border bg-white text-[#25D366] hover:border-[#25D366]/50 hover:bg-[#25D366]/5 transition-all active:scale-[0.95]"
              >
            ...
            <button
              type="button"
              onClick={handleChatClick}
              className="shrink-0 flex-1 min-w-0 py-3 rounded-xl font-bold text-white bg-brand-primary hover:brightness-90 active:scale-[0.97] transition-all duration-200 flex items-center justify-center gap-2 shadow-sm"
            >
