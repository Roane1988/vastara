...
- Skope hanya frontend (ChatHubPage, VoiceWaveform baru, audioCompression baru, index.css). Tanpa migration/DB (bucket `CHAT_VOICE` & kolom `direct_messages` sudah ada sejak changelog voice note sebelumnya). Lint bersih (`eslint`) + build sukses (`vite`). Commit: (lihat git).
