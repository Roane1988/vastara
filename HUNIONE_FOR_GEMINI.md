...
- **(2) Pause & Resume** (`handlePauseRecording` / `handleResumeRecording`): memakai `mediaRecorder.pause()` dan `mediaRecorder.resume()`. Durasi diakumulasi via `recordElapsedRef` + `recordStartRef` sehingga timer **tidak ikut berdetak** selama dijeda; batas 10 menit tetap diterapkan setelah resume.
- **(3) Delete / Batal**: tombol ikon Tong Sampah (Trash) memanggil `handleDiscardRecording()` — berhentikan recorder (`cancel()`/`stop()`), buang `audioChunks`, reset timer & durasi, matikan visualizer & stream mikrofon, lalu tutup mode rekaman **tanpa mengirim**. State `isPaused` ikut di-reset.
- **(4) Real-time Waveform (Visualizer)**: saat merekam, `<canvas>` baru (`recCanvasRef`) ditambahkan di sebelah timer. Helper `initRecVisualizer` membuat `AudioContext` + `AnalyserNode` dari `stream` mikrofon (fftSize 256) dan `drawRecVisualizer` menggambar **42 bar vertikal** gradien merah–rose setiap frame via `requestAnimationFrame` (dengan data `getByteFrequencyData`). Selama `isPaused`, animasi gelombang **berhenti menggambar** (frame terakhir dibekukan). `stopRecVisualizer` membersihkan RAF, memutus source, menutup AudioContext, dan mengosongkan canvas (dipanggil di send, discard, limit 10 menit, ganti kontak, dan unmount).
- **(5) Layout UI Perekaman (WhatsApp-style)**: di dalam pill komposer saat mode merekam aktif — **[🗑️ Trash]** di kiri, **[⏸️/▶️ Pause/Resume]** di sebelahnya, **[📊 Canvas Waveform]** di tengah (flex-1), lalu **[Timer]** + **[Kirim]** (tombol bulat pesawat kertas) di kanan. Saat dijeda, timer meredup (`text-brand-muted`) dan muncul label kecil **"dijeda"**; saat merekam timer berwarna merah (`text-brand-danger`).
- Skope hanya `ChatHubPage.jsx`. Tanpa migration/DB. Lint bersih (`eslint`) + build sukses (`vite`). Commit: `b77eada`.
...
- **(2) Eksekusi langsung tanpa pre-check**: `startRecording` kini **langsung memanggil** `navigator.mediaDevices.getUserMedia({ audio: true })` begitu tombol Mic ditekan. Pengecekan awal yang sebelumnya berjalan sebelum akses — cek HTTPS dan `getAudioInputDevices()` (enumerasi perangkat) sebagai *pre-block* — **dihapus** dari jalur sukses. Tidak ada banner/peringatan merah yang muncul di awal; banner `micError` di-reset (`setMicError('')`) tepat sebelum mencoba.
- **(3) Error hanya pada kegagalan riil**: pesan error (`setMicError` + `showToast`) hanya muncul di dalam blok `catch` `getUserMedia`, ketika sistem mengembalikan error asli. `micGuidanceFor(name)` kini berbasis **nama error sebenarnya** saja (`TimeoutRequestingMicrophone`, `NotAllowedError`/`PermissionDeniedError`/`SecurityError`) — tanpa cabang state `denied`/`prompt` dari permissions API. `getAudioInputDevices()` tetap dipakai **hanya sebagai diagnostik** di dalam `catch` untuk `NotFoundError`/`OverconstrainedError` (membedakan "tidak ada perangkat" vs "perangkat gagal"), bukan sebagai penjaga jalur sukses. Jika berhasil → perekaman berjalan normal tanpa banner.
- **Retry otomatis dipertahankan**: `scheduleMicRetry` tetap memberi satu percobaan ulang ~1,5s untuk error retryable (izin ditolak/timeout), dan `startRecording` pada retry juga mengulang `setMicError('')` dulu — tidak pernah terjebak banner lama.
- Skope hanya `ChatHubPage.jsx`. Tanpa migration/DB. Lint bersih (`eslint`) + build sukses (`vite`). Commit: `02994b4`.
